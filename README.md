...
## v4 수정
- 검사 결과에 항상 보이는 세로/가로 스크롤바 추가
- 결과 요약을 클릭 가능한 필터 버튼으로 변경
  - 전체 / 완전 동일 / 내용 동일 / 유사 / 읽기 실패
  - 기본 선택은 전체
  - 필터 전환 시 재검사하지 않고 기존 결과만 즉시 변경
- 같은 파일 조합의 반복 출력 제거
  - A-B / B-A 중복 방지
  - 동일 조합이 여러 판정 경로로 잡힌 경우 완전 동일 > 내용 동일 > 유사 우선
- 그룹 종류별 옅은 배경색과 간격/열 폭 정돈
- 전용 EXE 아이콘은 아직 포함하지 않음. 사용자 시안 확인 후 별도 적용 예정.
